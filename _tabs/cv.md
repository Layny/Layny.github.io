---
icon: fas fa-code
order: 5
title: CV
---

## Certifications

- **eJPT** — eLearnSecurity Junior Penetration Tester *(Juillet 2026)*
- **CPTS** — Certified Penetration Testing Specialist *(en préparation)*

## Compétences

**Administration systèmes**
- Windows Server / Active Directory
- Linux (Debian, RHEL)

**Sécurité opérationnelle**
- Durcissement de systèmes
- SIEM : Logpoint, ELK, Wazuh
- EDR : SentinelOne, HarfangLab
- Réponse à incident

**Sécurité offensive**
- Reconnaissance & énumération (Nmap, NetExec)
- Active Directory : BloodHound, Impacket, Rubeus, Kerberoasting / AS-REP Roasting
- Pivoting : Chisel, ligolo-ng
- Reverse shells & post-exploitation

**Automatisation**
- Python, PowerShell, Bash
- Ansible, Docker

**Hardware / Pentest physique**
- Flipper Zero (RFID/NFC, Sub-GHz)
- Analyse de trafic Wi-Fi (monitor mode)

## Langues

- Français — natif
- Anglais — technique

---

[![HackTheBox](https://img.shields.io/badge/-HackTheBox-9FEF00?style=for-the-badge&logo=hackthebox&logoColor=black)](https://app.hackthebox.com/users/1368058)
[![TryHackMe](https://img.shields.io/badge/-TryHackMe-212C42?style=for-the-badge&logo=tryhackme&logoColor=red)](https://tryhackme.com/p/Layny)
[![Root-Me](https://img.shields.io/badge/-Root--Me-2E3440?style=for-the-badge)](https://www.root-me.org/Layny)